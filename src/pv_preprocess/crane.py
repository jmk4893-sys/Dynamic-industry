...
#: 스팬/2 − 트롤리 접근 ≥ 밴드/2 이므로 최소 8,550 + 2×600 = 9,750 이다.
#: REV.57: 8,800 → 9,800. 반전 카세트를 장변 방향으로 돌리면서 밴드가
#: 7,100 → 8,550 이 됐다. **국부 확폭으로도 이 값은 안 줄어든다** — 런웨이는
#: 전장을 달리므로 한 군데라도 넓으면 스팬이 전 구간 그만큼 커진다.
SPAN_MM = 9_800

#: 주행 끝단 오버행 (mm) — 주행거더가 플랜트 양끝 밖으로 나가는 양. 엔드트럭과
#: 스토퍼가 이 안에 들어가고, 후크가 플랜트 끝단 설비 위까지 간다.
RUNWAY_OVERHANG_MM = 1_000

#: 주행거더 길이 (mm). 플랜트 전장에서 파생한다 — 존이 바뀌면 따라온다.
#: REV.45 이전에는 60,800 리터럴이었고, 통합으로 전장이 800 줄자 오버행이
#: 1,000 → 1,400 으로 늘어나 규칙과 어긋났다.
RUNWAY_MM = layout.plant_envelope_mm()[0] + 2 * RUNWAY_OVERHANG_MM
...
LIFTS: tuple[Lift, ...] = (
    ...
    Lift("AFR CL-221 클램프 포탈 1조", "afr", 700, kinematics.AFR_PORTAL_HEIGHT_MM,
         f"150×150 t9 기둥 4본 × {kinematics.AFR_PORTAL_HEIGHT_MM / 1000:.2f} m "
         f"(39.8 kg/m) · 크로스헤드 2 · 타이빔 2"),
