...
HEADS: tuple[VisionHead, ...] = (
    ...
    VisionHead("VS-301", "JB/AFR 게이트", "박리 후 검증 · 공유맵 생성", False,
               ...
               "3D 실측상 이 헤드는 JBR 셀 안쪽(x −2,832…−2,548)이라 제거해도 라인 길이는 줄지 않는다 — "
               "전장 900 mm 감소는 게이트 자리표시 치수를 실측 이격으로 바꾼 결과다"),
    VisionHead("GI-301", "유리 후단", "연마 전 잔사 라인스캔 검사", False,
               "연마 후 통합 검사대로 흡수 (V-4). CV-102 는 광학을 떼고 이송만 남아 3,700→2,800 mm. "
               "연마 전/후 비교와 패널별 연마 적응이 사라지므로 연마 공정창 고정이 전제다. "
               "REV.50: SG-301 이 AFR 반출롤러 위로 가고 CV-102 는 GI 밑 통과 롤러 런이 됐다 — "
               "포탈 자체가 없어져 은퇴 헤드 메시도 같이 갔다"),
    VisionHead("GI-302", "유리 후단", "연마 후 잔사＋결함맵＋레시피 통합 판정", True,
               "명시야 라인스캔 뱅크를 더해 한 갠트리에서 잔사와 레시피를 함께 본다 (V-4). "
               "2,000→2,400 mm. 유리 후단 외형은 순 −500 mm"),
    VisionHead("GI-303", "유리 후단", "연마 후 하부 유리면 균열·엣지 라인스캔", True,
               "REV.51. 유리면이 아래를 향한 채 후단을 지나므로 GI-302 는 백시트 쪽만 본다 — "
               "프레임 제거로 생긴 균열의 직접 증거가 없었다. CV-102 롤러(Ø90 · 피치 240) 사이 "
               "150 mm 틈에 라인조명과 라인스캔을 넣어 통과 중에 밑을 본다. 외부 R3 안의 "
               "하부 고정 스캔선을 받아들인 것이다"),
)
...
RETIRED_MESH_LABELS: tuple[str, ...] = (
    ...
    # "AFR CV-102/GI-301 다각도 라인스캔 헤드" 는 REV.50 에서 CV-102 포탈과 함께 없어졌다 —
    # 숨길 메시가 없으니 여기서도 뺀다.
)
