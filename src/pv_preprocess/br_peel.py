...
**2 차가 무엇을 가르는지는 아직 못 읽었다.** 도면이 「2 차 커팅」이라 적는데
나는 「칼날 진입·박리 시작」으로 적어 두었었다. 폭을 나누는 절단이면 박리력이
나뉘므로 사양이 달라진다 — `second_cut_is_unread()` 가 읽힌 것과 못 읽은 것을
갈라 둔다. **추측으로 메우지 않는다.**
...
#       ② 2 차 커팅 — **또 한 번의 절단이다.** 도면이 「2 차 커팅」이라 적고
#                      화살표가 아래를 가리킨다. 무엇을 가르는 절단인지는
#                      아직 못 읽었다 (`second_cut_is_unread()`).
#
...
def stages() -> tuple[tuple[str, str], ...]:
    ...
    return (
        ...
        ("② 2 차 커팅",
         "**절단이다 — 박리 진입이 아니다.** 도면이 「2 차 커팅」이라 적고 "
         "화살표가 아래를 가리킨다. 한때 이 자리를 「칼날이 틈으로 들어가 "
         "박리가 시작된다」로 적었는데 도면은 **커팅**이라고 한다. 무엇을 "
         "가르는 절단인지는 아직 못 읽었다 — `second_cut_is_unread()`."),
        ("③ 박리",
         f"백시트가 필름째 나온다. 폭 전체를 한 번에 벗기면 "
         f"{peel_force_n():,.0f} N 이고, 노후가 {aging_saves_us()} 배·가열이 "
         f"{heat_saves_us()} 배를 깎아 {heat_brings_it_to_n():,.0f} N 까지 "
         "내려온다. **다만 2 차 커팅이 폭을 나눈다면 이 힘도 그만큼 나뉜다** — "
         "그래서 ②를 못 읽은 채로 이 값을 확정으로 읽으면 안 된다."),
    )


def second_cut_is_unread() -> tuple[str, ...]:
    """**2 차 커팅이 무엇을 가르는지 아직 못 읽었다.** 추측을 적지 않는다.

    이 자리에서 이미 두 번 틀렸다 — 「JBR 절결을 주워 쓴다」, 그리고
    「30 mm 가 절단 길이」. 세 번째 추측을 모델에 넣지 않는다.
    """
    return (
        "**읽힌 것** — 도면이 ①을 「1 차 커팅」, ②를 「2 차 커팅」이라 적는다. "
        "둘 다 **절단**이고, ②의 화살표는 아래(↓)를 가리킨다. ①의 화살표는 "
        "옆(←)에서 상단 띠를 가리킨다.",
        "**못 읽은 것** — ②가 무엇을 가르는지. 폭 방향을 나누는 세로 절단인지, "
        "깊이를 더 넣는 이차 절입인지, 띠를 떼어 내는 절단인지 구분이 안 된다.",
        f"**왜 중요한가** — ②가 폭을 나누면 박리력이 그만큼 나뉜다. 지금 모델은 "
        f"폭 {starter_cut_length_mm():,.0f} mm 를 한 번에 벗기는 것으로 "
        f"{peel_force_n():,.0f} N 을 든다. 둘로 나뉘면 절반이고, 그러면 "
        "그리퍼·구동 사양이 통째로 달라진다.",
        "**그래서 ②의 힘 값을 지금 확정으로 쓰지 않는다.** 답이 오면 "
        "`stages()` 와 박리력만 고치면 되고, 1 차 커팅 쪽은 안 건드려도 된다.",
    )
...
def open_questions() -> tuple[str, ...]:
    ...
    return (
        ...
        "남는다.",
        "**2 차 커팅이 무엇을 가르는지 못 읽었다.** 도면이 ②를 「커팅」이라 "
        "적는데 나는 「칼날 진입·박리 시작」으로 모델링해 두었다. 폭을 나누는 "
        "절단이라면 박리력이 나뉘므로 그리퍼·구동 사양이 달라진다 — "
        "`second_cut_is_unread()` 가 읽힌 것과 못 읽은 것을 갈라 둔다.",
