...
STATIONS: dict[str, Station] = {
    ...
    for s in (
        ...
        Station("jbr", "PV-JBR-201-GA-3101", "JBR-201 · 케이블·정션박스 3헤드 제거셀",
                (7050, 3050, 2800), 900),
        # REV.22-P01 장비 단축 2건. 근거는 둘 다 도면 부품표 실측이다.
        #
        # 1. CV-101 투입롤러 3,700 → 2,800. 같은 2,500 mm 패널을 스테이션 사이로 옮기는
        #    순수 이송인데 V-4 로 광학을 뗀 CV-102 는 2,800 이라 같은 일에 두 규격이 있었다.
        #    JBR 출구 롤러가 정반 하류로 2,175 mm 남아 축적을 이미 맡는다.
        # 2. 가드 여유 균등화. 가드는 ±5,750 대칭인데 장비는 −5,550…+4,300 이라 상류 200 ·
        #    하류 1,450 이었다 — 하류 1,450 을 설명하는 정비 치수가 없다. 플랜트에서 가장
        #    넉넉한 X 가드 여유(유리 후단 475)를 양쪽에 적용해 상류는 오히려 넓어진다.
        #
        # 장비 −4,650…+4,300 (8,950) + 475×2 = 9,900.
