...
#:
#: 그래서 면을 **3D 에서 실측한 값**으로 잡는다 (장비 밴드 안, 시간을 훑어
#: 움직이는 것까지 포함한 최대 z → 플랜트 Y). §7 의 캐리지 피치 2,900 과
#: §26 의 링 관통 88 mm 을 실측으로 잡았던 것과 같은 자리다.
#:
#: **재는 일은 도구로 한다** — `node tools/measure_zone_faces.mjs`. 손으로 적어
#: 두면 밴드가 움직일 때 조용히 낡는다. 반전축을 장변 방향으로 돌리며 밴드가
#: 7,100 → 8,550 이 되자 라인 중심(월드 z 원점)이 플랜트 Y 3,550 → 4,275 로
#: 옮겨갔고, 옛 값은 725 mm 어긋난 프레임에서 잰 것이 됐다. 그 상태에서
#: `encroach_mm()` 이 전부 0 을 냈는데 그것은 밴드가 넓어져서가 아니라 값이
#: 낡아서였다 — 아래는 돌린 3D 에서 다시 잰 값이다.
MEASURED_FACE_MM: dict[str, int] = {
    "afu": 8115,      # BW-101 벽체 기둥 베이스플레이트 (벽 z 3,580 + 반폭 260)
    "robot": 7575,    # RB-101 명판
    "jbr": 8055,      # 존 안전펜스 포스트 (z 3,740)
    "afr": 7825,      # 존 안전펜스 포스트 (z 3,500)
    "post": 7805,     # GI-302 명판·존 가드 포스트
    "buffer": 7855,   # MB-041 지지 브래킷
    "grm": 7905,      # GRM 셀 베이스 빔 14,050 (z 3,550 + 80)
}
...
#: REV.57: 50,075 → 49,925. 반전 카세트를 장변 방향으로 돌리며 페데스털이 상류로
#: 150 왔고 (도달 사슬), 전장이 그만큼 줄었다.
MEASURED_END_MM = 49_925

def scene_end_shim_mm() -> int:
    """3D 끝단 판이 존 경계 밖으로 **더** 물러서야 하는 양 (mm). 등록되면 0.

    존 표는 **설계**고 3D 는 그 설계를 비추는 **그림**인데, 한동안 둘의 원점이
    맞물려 있지 않았다 (`layout.SCENE_GRID_OPEN`, REV.48 에서 해소). REV.44
