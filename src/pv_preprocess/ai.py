...
CASES: tuple[Case, ...] = (
    ...
    Case("AI-01", "투입 3분류 (정상·유리 깨짐·전손)", "A", "엣지 추론 서버",
         ...
         "사전학습 CNN 전이학습 · 3클래스",
         f"전손 오통과 1장 = 병목 {scrap_miss_cost_s():g} s, 파손유리 오통과 "
         f"1장 = 후단 데크 피치 {cracked_miss_cost_s():g} s. 지금은 규칙기반이라 "
         f"애매한 장을 사람이 본다.",
    ...
    Case("AI-09", "라인 스케줄 강화학습", "C", "해당없음",
         ...
         "—",
         "—",
         # 병목 이름·시간을 문장에 박아 두면 설계가 움직일 때 조용히 늙는다 —
         # REV.57 까지 실제로 "GRM-401 46.49 s" 로 굳어 있었고, REV.58 에서
         # JBR 이 54 s 가 되며 거절 근거가 다른 설비를 가리켰다. 모델에서 낸다.
         f"**권고하지 않는다.** 이 라인은 직렬이고 병목이 "
         f"{campaign.bottleneck()} {campaign.ideal_takt_s():g} s/장 로 "
         f"고정이라 최적해가 자명하다. 실제 택트 "
         f"{campaign.summary()['takt_s']:g} s 와 그 병목 사이에 남은 것은 "
         f"{campaign.summary()['takt_s'] - campaign.ideal_takt_s():.2g} s 뿐이고, "
         f"그마저 앞 장 스토퍼 해제 규칙이 정한 값이라 스케줄이 아니라 기구가 "
         f"쥐고 있다. 학습 정책을 넣으면 검증·설명 부담만 는다."),
