...
from . import campaign, handoff, hk60c, reliability, smart

#: CL-01 포락선 문장에 들어가는 수 — 아래 envelope_bounds() 와 같은 출처다. 문장에
#: 리터럴을 적으면 후단 기계가 바뀔 때(REV.54 DG-HK60C) 문장만 옛 값에 남는다.
...
CASES: tuple[Case, ...] = (
    ...
    Case("AI-01", "투입 3분류 (정상·유리 깨짐·전손)", "A", "엣지 추론 서버",
         ...
         "되돌려 붙이려면 패널 ID 가 버퍼·유리제거기까지 이어져야 한다 — RF-901/902 "
    ...
    Case("AI-06", "IR 가열 종점 최적화", "B", "엣지 추론 서버",
         "OPC-901 DG-HK60C 계면 열전대(캐리지별 3점)·IR 뱅크 전력 구독 · RF-902 패널 ID 인계",
         "박리 결과(AI-07 의 완전도 판정)가 그대로 라벨이 된다 — 닫힌 고리다.",
         "데크별 열이력 → 박리 성공 확률 모델. 장당 잔여 소성시간을 내고 "
         "**IR 뱅크 출력을 직접 조절한다** — 이 플랜트의 유일한 폐루프(CL-01). "
         f"포락선(출력 {_BANK_KW:g} kW·소성 {_DWELL_MIN_S:g}…{_DWELL_S:g} s)은 모델 밖 인터록이 지키고, "
         f"센서·모델 이상이면 고정 {_DWELL_S:g} s 로 복귀한다.",
         f"지금은 소킹 {_DWELL_S:g} s 를 **전 장에 똑같이** 준다. 유리 두께·오염도가 "
         "다른데도 같은 시간을 주는 것이라, 짧게 끝낼 수 있는 장에서 시간을 "
         f"버리고 있다. 병목은 탠덤이지만 IR 여유({_IR_HEADROOM_PER_H:g} 장/h)가 줄면 병목이 "
         "넘어온다 — 여유를 지키는 쪽으로도 값어치가 있다.",
         "계면 온도는 벤더 PLC 가 잰다 — OPC UA 로 **읽을** 수는 있지만 레시피 "
         "**쓰기**는 인증서 제한이다 (사양서 7.x). 폐루프는 벤더 협의 항목이고, "
         "그 전까지는 권고만 낸다.",
         ("OPC-901", "RF-902")),
    Case("AI-07", "박리 완전도 판정", "B", "엣지 추론 서버",
         "DG-HK60C QI-301 상부 RGB 카메라 판정 (OPC-901 구독)",
         "후단 시트 유리 회수 성공/실패. 공정이 만든다.",
         "잔막 영역 세그멘테이션 + 합격/재투입 2분류",
         "REV.54: 유리제거기 출구 검사는 벤더 QI-301 이 한다 (GLASS_CRACK·QI_FAIL). "
         "그 판정이 AI-06 의 라벨이기도 하다 — 영상 원본은 벤더 안에 있고 우리는 "
         "판정 결과와 리젝트 이력을 받는다.",
    ...
    Case("AI-09", "라인 스케줄 강화학습", "C", "해당없음",
         ...
         "—",
         "—",
         f"**권고하지 않는다.** 이 라인은 직렬이고 병목이 {campaign.bottleneck()} "
         f"{campaign.ideal_takt_s():.2f} s/장 로 고정이라 최적해가 자명하다. 실제 택트 "
         f"{campaign.summary()['takt_s']:.2f} s 는 그 병목에서 후단 제한 택트로 페이싱한 "
         "값이라 스케줄이 아니라 인터록 하나(방출 보류)가 쥐고 있다. 학습 정책을 "
         "넣으면 검증·설명 부담만 는다."),
...
def envelope_bounds() -> dict[str, float]:
    ...
    from . import handoff
    return {
        "maxBankKw": round(handoff.LAMP_COUNT * handoff.LAMP_KW, 1),
...
LOOPS: tuple[ControlLoop, ...] = (
    ControlLoop(
        tag="CL-01",
        case="AI-06",
        measured="OPC-901 DG-HK60C 계면 열전대·IR 뱅크 실전력 구독 (1 Hz) · RF-902 패널 ID",
        actuated="IR 뱅크 6뱅크 SSR 출력 — 벤더 레시피 쓰기(인증서 제한) 협의 후",
        setpoint=f"데크별 잔여 소성시간 — 종전의 전 장 고정 {_DWELL_S:g} s 를 장마다 다르게 준다",
        envelope=f"뱅크 출력 상한 {_BANK_KW:g} kW (설치용량) · 소성 하한 {_DWELL_MIN_S:g} s (열전달 "
                 f"하한) · 소성 상한 {_DWELL_S:g} s (현행 고정값 — 모델은 줄이기만 한다). "
                 "셋 다 안전 PLC 가 지키고 모델이 못 넘는다. 온도 스위치는 별개 "
                 "층이고 설정값은 아직 없다 (TEMP_TRIP_OPEN)",
        rate_limit="출력 변화율 ±10 %/10 s. 급변은 유리에 열충격을 준다",
        fallback="OPC-901 계면 열전대 3점 중 2점 이상 이상 · 모델 무응답 2 s · 예측이 포락선 밖 → "
                 f"고정 {_DWELL_S:g} s 로 즉시 복귀하고 콘솔에 사유를 띄운다. 폴백은 성능 "
                 "저하이지 정지가 아니다",
