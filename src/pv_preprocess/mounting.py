...
#:
#: 46 → 43. 반전축을 장변 방향으로 돌리며 투입 무리가 통째로 움직였고, 도구를
#: 다시 돌리니 매달 틈이 셋 줄었다. 값을 손으로 고른 것이 아니라 출력이다.
#:
#: 43 → 42. 버퍼 캐리지 행을 제약에서 다시 내며(∓2,300) 콤포크가 200 mm 밖으로
#: 나갔고, 그 자리에서는 매달 틈이 하나 줄었다. 이 값도 도구 출력이다.
BRACKET_COUNT = 42
