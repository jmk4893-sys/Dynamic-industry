...
_SHARES: tuple[tuple[str, str, float, bool, str | None, str], ...] = (
    ...
    ("RB-JBR", "JBR-201 1헤드 순차 제거", 0.20, False, "stock",
     "**마모부가 가장 많다** — 칼날·가위·에어나이프·프로브. 헤드 하나가 박스 수만큼 반복해 마모가 집중된다. "
    ...
    ("RB-GRM", "DG-HK60C 유리제거기 (벤더)", 0.12, False, "headroom",
     "IR 램프 40개·핫나이프 카세트·갠트리·진공. 램프 1개 고장은 감속 운전이라 "
     "완전정지가 아니다. 벤더 보증 가동률 0.90 의 정지 예산(시간당 360 s)은 카세트 "
     "교환·롤 반출 같은 **계획 정지**라 순생산 60.5 에 이미 들어 있고, 여기 예산은 "
     "고장이다. 버퍼 하류라 여유공간이 흡수한다 — 전단이 그 동안 계속 돈다"),
...
def SPARES() -> tuple[Spare, ...]:
    ...
    return (
        ...
        Spare("SP-02", "칼날 카세트 BC-201 (DG-HK60C KC-101 매거진)", "RB-GRM", 2, None, 8,
              "HKB/HKS 한 벌. 수명은 파일럿 PT-02 (사양서 OI-12) — 268 장마다 한 번보다 "
              "잦게 갈리면 계약 60 장/h 가 무너진다"),
        ...
        Spare("SP-10", "IR 램프 (DG-HK60C HC-101 · 6뱅크 · 발열장 2,200)", "RB-GRM",
              handoff.LAMP_COUNT, None, 12,
              f"{handoff.LAMP_COUNT}등. 단자가 측벽 밖 소켓이라 챔버를 안 열고 간다. "
              "램프 1개 고장은 감속 운전이라 완전정지가 아니지만, 누적되면 처리량이 "
              "내려간다. 수명은 벤더값"),
    )
