...
SERVO_AXES: tuple[Axis, ...] = (
    ...
    # REV.57: 헤드 3 기 동시 → 1 기 순차. 한 헤드가 박스를 하나씩 찾아가므로
    # Y 는 1 축이면 되고, 박스 기울기(시나리오 최대 ±1.03° · coverage 5.2°)를 따라가려면
    # **구동 요축**이 있어야 한다 — 패시브 ±0.5° 로는 안 된다.
    Axis("AXIS-JBR-HY", "LP-JBR", "JBR-201", "헤드 Y", 1, 0.2,
         "서보", "23 bit 절대", False, "0.2 kW 서보벨트 모듈 — 박스마다 Y 로 찾아간다"),
    Axis("AXIS-JBR-C", "LP-JBR", "JBR-201", "헤드 요 C", 1, 0.2,
         "서보", "23 bit 절대", True,
         "박스 기울기 추종 ±5° — 순차 1 헤드라 요를 구동해야 모든 자세를 잡는다"),
    # 박리축은 서보를 뺀다. 절입 깊이는 POM 기준 슈가 기계적으로 잡고, 상용 제거기도
    # 공압으로 민다 — 실린더는 작업력을 몰라도 수명이 주행거리로 정해진다.
    ...
    Axis("AXIS-SG-T", "LP-AFR", "SG-301", "단변 헤드 횡행 (폭 1,400)", 1, 0.4,
    ...
    #
    # REV.53: GBR→GRM 950 mm 구간에 아무 축도 없었다 — layout.NON_ROLLER_HANDOFF
    # 가 "캐리지를 통째로 옮긴다"고 적어 뒀지만 서보도 부품도 없이 문장뿐이었다.
    # M1-101 은 롤링(한 장 빠지면 한 장 채움)이라 통째 교체는 배치식으로 바꾸는
    # 셈이고, 그러면 downstream_rate() 처리율(외부 DG-HK 2400 앱 값, 재유도하지
    # 않는다)을 다시 내야 한다. 그래서 캐리지는 그대로 두고 **한 장씩** 건넨다.
    Axis("AXIS-GRM-BX", "LP-GRM-MEC", "BX-101", "GBR 인계 브리지", 1, 0.75,
         "서보", "23 bit 절대", False,
         "GBR 캐리지 슬롯 앞 950 mm 공백을 건너 M0-101 상면(1,050)에 내려놓는다 — "
         "M0-101 자세·중심 확인은 그대로 두고, 그 앞 950 을 새로 잇는다"),
