...
FEEDERS: tuple[Feeder, ...] = (
    ...
    # 통합셀 안으로 들어왔다 — 한 베이스·한 가드인 기계에 반이 둘일 이유가 없다.
    # 수용률이 둘 다 0.70 이라 수요(16.59 kW)는 한 자리도 안 바뀐다. 바뀌는 것은
    # 반 하나(7면 → 6면)와 피더 하나, 그리고 그 반까지 가던 케이블 한 본이다.
    # 23.7 kW 는 380 V·역률 0.9 에서 40.0 A — 32 AT 로는 못 받아 63 AT/16 mm² 다.
    Feeder("F4", "LP-AFR",
    ...
    # REV.59: SG-301 장변 2 대가 동시라 국소 후드가 두 몫이다 (1,000 → 2,000).
    # 주 블로워 7.5 kW 는 GA 가 1,000 m³/h 로 적어 둔 것이라 **여기서 모자란다** —
    # `dust.blower_is_undersized()` 가 그것을 값으로 내놓는다.
    Feeder("F7", "LP-DX", "DX-601 집진 2,000 m³/h · JBR 국소집진 350 m³/h",
           11.0, 0.90, 32, "4C×6 mm² Cu", "GA 명시(풍량) · 풍량 재산정 필요"),
    ...
    # REV.34: 압축공기. 이 플랜트는 REV.33 까지 공압을 **쓰면서** 만드는 것을
    # 안 세우고 있었다 — JBR 셀만 평균 260 / 피크 420 NL/min 이다.
    # 수용률은 상수가 아니라 air.diversity() 가 낸다(1운전/2대 × 부하율).
    Feeder("F16", "LP-AIR", "CMP-701 압축공기 (스크류 5.5 kW × 2 — 1운전 1예비) "
           "· 냉동식 드라이어 · 리시버 300 L",
           11.5, 0.38, 30, "4C×6 mm² Cu", "계획(air.installed_kw)"),
)
