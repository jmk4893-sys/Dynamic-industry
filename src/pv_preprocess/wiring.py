...
#: REV.45 에서 **41,500 으로 내려왔다.** JBR·AFR 을 한 가드로 묶으며 게이트 존
#: 350 이 없어지고 셀 외형이 순 −800 이 되어, 하류 부하(IR 뱅크·시설 블록)가 전부
#: 800 씩 상류로 당겨졌다. 부하중심 42,197 → 41,683 이고 500 단위 반올림이 41,500 이다.
#: 되먹임(반 → 시설 → LP-IT → 부하중심)의 고정점이며, 2회 반복에서 수렴한다.
#: REV.49 에서 **40,000.** 반전 드럼이 적층 위로 오며 afu 존이 1,850 줄었고 하류
#: 부하가 전부 그만큼 상류로 왔다. 부하중심 41,683 → 40,065 이고 500 단위가 40,000 이다.
#: REV.50 에서 **36,000.** SG-301 이 AFR 반출롤러 위로 올라가며 후단 셀이 8,900 → 3,350
#: 으로 줄었고(AFR 은 +100) 하류 부하가 순 5,450 상류로 왔다. 부하중심 40,065 → 35,838
#: 이고 500 단위 반올림이 36,000 이다. 규칙은 그대로다.
MDB_POSITION_MM = (36_000, 8_150)
