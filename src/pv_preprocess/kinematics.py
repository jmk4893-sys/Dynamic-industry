...
#:
#: 지어낸 이야기가 아니다. 같은 회전을 따로 한 브랜치의 도면을 실측했다
#: (공정시계 0…124 s 를 0.5 s 로 훑음, `tools/check_bay_clearance.mjs`):
#:
#:   ============= ================== ================== ==========
#:   도면            Bay A z            Bay B z            틈
#:   ============= ================== ================== ==========
#:   회전 전         (기준선)                                 442
#:   이 브랜치       −3,675 … −125      −45 … +3,175         **80**
#:   피치 미유도     −3,050 … **+240**  +150 … +3,440      **−90**
#:   ============= ================== ================== ==========
#:
#: 마지막 줄은 `BFC-101A 서보·감속기·이중 브레이크` 덩어리가 중앙을 넘은 것이고,
#: 그 상태에서 `check_cell_grid`·`check_clearance`·`check_load_path` 는 **전부
#: 초록이었다.** 셋 다 베이 대 베이 가로 겹침을 보지 않기 때문이다. 그래서 검사를
#: 하나 더 뒀다.
#:
#: **좁은 배치를 다시 제안하지 말 것.** 폭을 안 넓히는 대신 두 베이를 거울상으로
#: 놓는 안이 있었고(지게차가 A/B 를 반대쪽에서 도킹한다), 발주처가 2026-09-08 에
#: **같은 방향·폭 확대**를 골랐다 — 지게차 동선을 하나로 두는 쪽이다. 대가는
#: 외벽 ∓3,250 → ∓3,580 과 크레인 스팬 8,800 → 9,800 이고, 그 값들이 이미
