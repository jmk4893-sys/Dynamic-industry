...
# ── 집진 탈진 (DX-601) ───────────────────────────────────────────────────
#: 집진 풍량 (m³/h) — SG-301 국소 2,000 + JBR 국소 350. `dust.counted_flow_m3h()`
#: 및 `electrical.FEEDERS` 의 F7 사양과 같아야 한다 (시험이 대조한다).
#:
#: REV.59 까지 1,350 이었다. SG 몫을 1,000 으로 잡은 근거가 "헤드가 순차라
#: 동시에 도는 것이 없다" 였는데, 캠페인 모델은 두 장변을 **한 번의 통과**로
#: 세고 있었다 — 그러려면 2 대가 동시다. 발주처가 동시로 정해 첨두가 후드 두
#: 몫이 됐고, 그것이 여과면적 → 펄스밸브 → 압축공기 → 컴프레서까지 간다.
DUST_FLOW_M3H = 2_350
