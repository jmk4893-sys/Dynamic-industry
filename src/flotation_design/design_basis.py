...
# --------------------------------------------------------------------------
#: 로드밀 배출을 고농도 그대로 받아 입자끼리 문질러 표면을 벗기는 설비.
#: 상세 근거와 계산 논리는 ``attrition.py`` 모듈 도크스트링 참조.
ATTRITION_TAG = "AS-1"
ATTRITION_DUTY = "어트리션 스크러버 (표면 정정) — 2셀 직렬"
...
#: 임펠러 — 대향 피치 축류 2단/축. 위는 아래로, 아래는 위로 밀어 중간
#: 높이에 전단면을 만든다. 조 폭 대비 지름비가 부선셀 로터(0.35)보다 큰 것은
#: 조 전체를 움직여야 하기 때문이다.
...
#: 실리카사 표면 정정의 통상 범위. 이 원료의 최적값은 시험(T-1)으로 정한다.
...
#: **성능 크레딧 없음.** [1][2] 어디에도 어트리션 시험이 없으므로 회수율·품위·
#: 약제 투입량 모두 어트리션이 없는 것과 같은 값으로 계산한다. 시험(T-1~T-3)
#: 에서 이득이 확인된 뒤에만 이 값을 올린다. 스캐빈저 포수제 증량
#: (``MECHANICAL_SCAVENGER_BOOST``) 과 같은 취급이다.
ATTRITION_PERFORMANCE_CREDIT = 1.0

#: 상방 시나리오 점검용 복합입자 동반비 r. 어트리션이 Ag 박편을 떼어내면
#: r 이 내려가고 정광 품위 상한 1/(1+r) 이 올라간다.
ATTRITION_CARRY_RATIO_CASES = (COMPOSITE_CARRY_RATIO, 0.8, 0.6, 0.4)

#: 합격 기준 — 스크러빙 전후 -10 um 질량분율 증가 허용치 (%p).
#: 이 이상 늘면 표면 정정이 아니라 분쇄를 하고 있다는 뜻이다.
...
#: 시료 1점 (건조) — 1 L 부선 1회분(7 wt% 로 약 75 g, AS-1 T-1 겸용) +
#: 수중 부침 분리(EVA)·TGA·입도·assay 몫 + 재시험 예비.
...
#: EVA 에 섞인다. AS-1(고무 라이닝)과 의도적으로 다른 유일한 점이다.
...
#: 역산해 판정한다. 실제 발생률은 P-0 밀폐 용기 시험으로 잰다.
PILOT_VENT_M3H = 30.0
...
#: 판정 기준 — 부착 EVA 제거율. **잠정값.** Ag 정광 품위나 Si 제품 규격이
#: 정해지면 거기서 역산한 값으로 바꾼다. 회분 → 연속 환산의 기준점이기도 하다.
