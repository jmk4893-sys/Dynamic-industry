...
#: 첨부 문헌은 러퍼와 러퍼-클리너 시험만 수행했으며 스캐빈저의 별도
#: 체류시간·대형화 자료는 없다. 따라서 기본설계에서는 제작비와 예비품을
#: 줄이도록 러퍼와 **같은 동체**를 사용한다. 스캐빈저는 포말층을 50 mm로
#: 얕게 하고 급기를 조금 높여 회수 위주로 운전한다. 얕은 포말층 때문에
#: 같은 동체에서도 유효 슬러리 체적과 실제 체류시간은 러퍼보다 약간 크다.
#: locked-cycle 시험에서 추가 체류시간의 이득이 확인될 때만 2차 스캐빈저를
#: 직렬 증설한다.
SCAVENGER_CELL = _cell(1.00, 1.30, 0.050, GAS_HOLDUP)
...
#: 단별 최소 체류시간 (분, 최대 처리량 기준). 문헌의 회분식 최적시간 3분에
#: 실기 스케일업 여유 2배를 적용한다. 스캐빈저만 8분으로 두던 이전 값은
#: 실험 근거가 없어 제거했다.
MECHANICAL_RESIDENCE_MIN = {"FC-201": 6.0, "FC-202": 6.0, "FC-203": 6.0}
...
IMPELLER_ASSEMBLY_MASS_KG = {"FC-201": 15.0, "FC-202": 15.0, "FC-203": 3.0}
...
# --------------------------------------------------------------------------
# 전처리 — 어트리션 스크러버 (공통 설비, 두 안이 함께 쓴다)
# --------------------------------------------------------------------------
#: 로드밀 배출을 고농도 그대로 받아 입자끼리 문질러 입자 표면의 EVA 를 떼는 설비.
#: 은을 띄우는 것은 뒤의 부선조 몫이다 — 이 설비의 성능은 부착 EVA 제거율과
#: 미립 생성량으로만 판정한다.
#: 상세 근거와 계산 논리는 ``attrition.py`` 모듈 도크스트링 참조.
ATTRITION_TAG = "AS-1"
ATTRITION_DUTY = "어트리션 스크러버 (EVA 박리) — 2셀 직렬"

#: 스크러빙 고체 농도. 어트리션의 성패를 가르는 **단 하나의 변수**다.
#: 70 wt% 는 이 원료(고체 비중 2.37)에서 고체 체적분율 약 50 vol% 에 해당하며,
#: 이 정도라야 입자끼리 닿아 마찰이 생긴다. 실리카사 스크러빙의 표준 조건
#: (70~75 wt%) 이 체적분율로 같은 값이 되는 것과 일치한다.
ATTRITION_SOLIDS_WT = 0.70

#: 이 아래로 묽어지면 어트리션이 아니라 그냥 교반이다.
ATTRITION_MIN_SOLIDS_VOLUME_FRACTION = 0.40

#: 상류 로드밀 배출 농도 요구값 — 설계 스크러빙 농도와 같다. 봉형밀 배출은
#: 통상 65~75 wt% 라 대개 그대로 받을 수 있지만, 미달하면 어트리션 앞단에
#: 사이클론 탈수를 넣어 올려야 한다. **상류 설비 계약의 인터페이스 조건**이다.
#: 절대 하한(체적분율 40 vol% 환산값)은 비중에서 직접 계산한다 —
#: ``AttritionScrubber.minimum_solids_mass_fraction``.
ATTRITION_MILL_DISCHARGE_MIN_SOLIDS_WT = ATTRITION_SOLIDS_WT

#: 직렬 셀 수. 한 조로 다 채우면 급광의 39 % 가 평균 체류시간의 절반도
#: 못 채우고 빠져나간다 (완전혼합조 RTD). 2단으로 26 % 까지 줄인다.
ATTRITION_CELLS = 2

#: 설계 총 체류시간 (최대 처리량 기준). 표면 정정 목적의 통상 범위 5~15 분의
#: 중앙값. 실제 체류시간은 상용 최소 기종 때문에 이보다 길게 나온다.
ATTRITION_RESIDENCE_MIN = 10.0

#: 팔각조 형상 — 깊이/폭 비와 여유고.
ATTRITION_DEPTH_TO_WIDTH = 1.2
ATTRITION_FREEBOARD_M = 0.10

#: 임펠러 — 대향 피치 축류 2단/축. 위는 아래로, 아래는 위로 밀어 두 흐름이 중간
#: 높이에서 부딪히는 **충돌면**을 만든다 (유체 전단이 아니라 입자끼리 문질리는
#: 곳이다). 조 폭 대비 지름비가 부선셀 로터(0.35)보다 큰 것은 조 전체를 움직여야
#: 하기 때문이다. 동력수 0.8 은 모터 예비 선정용 **설계 가정**이다 — PAS-1 의
#: 토크 실측으로 바꾼다.
ATTRITION_IMPELLERS_PER_SHAFT = 2
ATTRITION_IMPELLER_RATIO = 0.50
ATTRITION_POWER_NUMBER = 0.80

#: 설계 주속과 VFD 조정 범위 (m/s). 하한은 고농도 층을 움직이는 데 필요한
#: 최소값, 상한은 마모가 분쇄로 넘어가지 않게 하는 실무 한계다. 실제 VFD
#: 상한은 모터 정격에서 다시 계산한다.
ATTRITION_DESIGN_TIP_SPEED_M_S = 7.0
ATTRITION_TIP_SPEED_RANGE_M_S = (6.0, 9.0)

#: 비에너지 목표 범위 (kWh/t) — 어트리션의 **실제 제어변수**.
#: 실리카사 표면 정정의 통상 범위라 출발점일 뿐이다. 이 원료에서 EVA 를 떼는
#: 데 드는 값은 시험 셀 PAS-1(T-1)로 정한다.
ATTRITION_SPECIFIC_ENERGY_RANGE_KWH_T = (1.0, 6.0)

#: 셀 체적당 동력 (kW/m3) — 상용 어트리션 셀의 통상대. 비에너지와 독립적으로
#: 계산되므로, 둘이 동시에 범위에 들면 기종 선정이 서로 맞물린다는 뜻이다.
ATTRITION_SPECIFIC_POWER_RANGE_KW_M3 = (8.0, 25.0)

#: 임펠러 조립체 질량 예비값 — 임펠러 1개당 ``계수 x D^3``. 로터동역학
#: 검산용이며, 제작도 확정 후 실측 질량으로 재검증해야 한다.
ATTRITION_IMPELLER_MASS_COEFF_KG_M3 = 500.0

#: 액면에서 구동 데크까지의 축 여유 길이.
ATTRITION_SHAFT_LENGTH_MARGIN_M = 0.50

#: 마모 방호. Si 는 모스 경도 6.5 대의 각진 입자라 미끄럼 마모가 심하다.
ATTRITION_LINER = "천연고무 12 mm (임펠러 폴리우레탄 피복)"

#: 중력 급광. 50 vol% 슬러리는 원심펌프로 보낼 수 없다. 로드밀 배출 높이가
#: 부족하면 일축 편심(PC) 펌프를 넣어야 하며, 그때만 이 값을 올린다.
ATTRITION_FEED_PUMP_KW = 0.0

#: **부선 성능 크레딧 없음.** 어트리션은 EVA 박리 설비이고, 은을 띄우는 것은
#: 부선조가 한다. 부선 계산(회수율·품위·약제 투입량)은 문헌 원료 기준이며
#: EVA 를 뗀 효과를 반영하지 않는다. 스캐빈저 포수제 증량
#: (``MECHANICAL_SCAVENGER_BOOST``) 을 근거 없이 넣지 않은 것과 같은 취급이다.
ATTRITION_PERFORMANCE_CREDIT = 1.0

#: 합격 기준 — 스크러빙 전후 -10 um 질량분율 증가 허용치 (%p).
#: 이 이상 늘면 EVA 를 떼는 것이 아니라 Si 를 갈고 있다는 뜻이다.
ATTRITION_FINES_ACCEPTANCE_PP = 2.0

#: 희석박스 — 어트리션 배출(70 wt%)을 부선 농도로 묽히는 지점.
#: 어트리션 바이패스 배관도 여기로 합류하고, 밀도계로 희석수를 조절해
#: 조건조 급광 농도를 고정한다.
DILUTION_BOX_TAG = "DB-1"
DILUTION_BOX_DUTY = "희석박스 (스크러빙 → 부선 농도 · 바이패스 합류)"
DILUTION_BOX_RESIDENCE_MIN = 2.0

#: 연간 운전시간 — 약제 절감 등 연 단위 환산의 기준.
ANNUAL_OPERATING_HOURS = 8000.0

# --------------------------------------------------------------------------
# 파일럿 — PAS-1 블랙파우더 EVA 박리 시험 셀 (AS-1 시험 T-1 겸용)
# --------------------------------------------------------------------------
#: 제작해서 시험하는 장비다. 플랜트 설비가 아니므로 설치 전력·물수지에 넣지
#: 않는다. 상세 근거는 ``attrition_pilot.py`` 모듈 도크스트링 참조.
PILOT_TAG = "PAS-1"
PILOT_DUTY = "파일럿 어트리션 셀 (회분식) — 블랙파우더 EVA 박리 · AS-1 비에너지 확정"

#: 시험 분획 — 체 분급 31~75 µm. 설계 급광(P80 66 µm)과 같은 크기대다.
PILOT_FEED_SIZE_UM = (31.0, 75.0)

#: 시료 1점 (건조) — 수중 부침 분리(뜬 EVA / 가라앉은 분획) 2회분(각 100 g) +
#: TGA·입도·성분 분석 몫 + 재시험 예비.
PILOT_SAMPLE_DRY_KG = 0.30

#: 바닥 시료 밸브 — **플러시 바텀**(밸브 시트가 조 바닥 면에 있는 형식). 일반
#: 볼밸브는 노즐과 볼 안에 교반되지 않은 슬러리가 고인다. 퍼지는 실측 데드
#: 볼륨 이상으로 하고, 뺀 고체는 비에너지 분모(조 안 건조 고체)에서 뺀다.
PILOT_SAMPLE_VALVE = "DN50 플러시 바텀 밸브"

#: 회분당 채취 비에너지 (kWh/t). 0 은 투입 직후. 상한 20 은 AS-1 이 VFD
#: 상한에서 낼 수 있는 연속 비에너지의 약 3배 — 곡선이 포화하는 데까지 본다.
PILOT_ENERGY_POINTS_KWH_T = (0.0, 1.0, 2.0, 5.0, 10.0, 20.0)

#: 시료를 빼서 줄어드는 회분의 허용 한도. 회분 질량은
#: ``채취점 수 x 시료 질량 / 이 값`` 을 5 kg 단위로 올려 정한다.
PILOT_MAX_WITHDRAWAL = 0.10
PILOT_BATCH_ROUND_KG = 5.0

#: 시험 주속 — AS-1 의 VFD 하한 · 설계 · 실무 상한. 회전수는 AS-1 설계
#: 주속에서 정하고, 모터는 상한 주속에서 고른다.
PILOT_TIP_SPEEDS_M_S = (6.0, ATTRITION_DESIGN_TIP_SPEED_M_S, 9.0)

#: 현장 전원 주파수 (Hz) — 3상 380/220 V 60 Hz (상류 DG-HK60 RFQ 전기부하표와
#: 같은 현장). PAS-1 은 토크센서를 축 사이에 넣느라 감속기 없이 **직결**하므로,
#: 모터의 기저속도(= 극수와 주파수가 정하는 동기속도)가 시험 회전수 범위와 맞아야
#: 한다. AS-1 은 감속기가 있어 출력 기준 선정으로 충분하다.
SITE_SUPPLY_HZ = 60.0
#: 직결 모터 극수 후보. 최고 시험 회전수에서 정격 출력이 나오는(기저속도가 그
#: 아래인) 가장 적은 극수를 고른다.
PILOT_MOTOR_POLE_SERIES = (2, 4, 6, 8)
#: VFD 과부하 — 정격 토크의 150 % 를 60 s (중부하 정격). 굳은 슬러리 기동에 쓴다.
PILOT_VFD_OVERLOAD = 1.5
#: 약계자(기저속도 위, 정출력) 운전 상한 — 기저속도의 1.5 배. 더 올리면 최대
#: 토크가 모자란다.
PILOT_MAX_FIELD_WEAKENING = 1.5

#: 시험 온도 (°C) — 기준 20, P-3 에서 10·30. EVA 는 이 범위 전부에서 고무
#: 상태(Tg 약 -30 °C)다. 목적은 효과 확인보다 **온도를 고정해 비에너지와
#: 뒤섞이지 않게** 하는 것이다.
PILOT_TEMPERATURES_C = (10.0, 20.0, 30.0)

#: 형상 — AS-1 과 기하 상사(깊이/폭, 임펠러 지름비·단수·동력수, 여유고)를
#: 그대로 쓴다. 아래 두 값은 D 에 대한 비: 하단 임펠러 바닥 간극, 시료를 다
#: 뺀 뒤에도 상단 임펠러 위에 남아야 하는 액 깊이.
PILOT_IMPELLER_CLEARANCE_RATIO = 0.5
PILOT_MIN_SUBMERGENCE_RATIO = 0.5

#: 구동부(모터·토크센서·베어링 하우징)가 뚜껑 위 브리지에 앉는 높이만큼의
#: 축 여유 길이. AS-1(구동 데크 0.5 m)보다 짧다.
PILOT_SHAFT_LENGTH_MARGIN_M = 0.25

#: 재킷 총괄 열전달계수 (W/m2K) — 고농도 슬러리 강교반, 재킷 측 글리콜.
#: 보수적으로 낮게 잡았다.
PILOT_JACKET_U_W_M2K = 350.0
#: 온도조절기(TCU) 냉매 최저 공급 온도 (°C) — 글리콜 30 % 수용액 칠러 기준.
PILOT_TCU_MIN_SUPPLY_C = -5.0

#: 토크센서 정격 계열 (N·m). 최대 운전 토크에 기동 서비스계수(2.0)를 실은
#: 값을 담는 최소 정격을 고른다.
PILOT_TORQUE_SENSOR_SERIES_NM = (5.0, 10.0, 20.0, 50.0, 100.0)

#: 접액부 재질. **고무·우레탄 금지** — 마모분이 유기물이라 TGA 로 재는 잔류
#: EVA 에 섞인다. AS-1(천연고무 라이닝 · 우레탄 피복 임펠러)과 의도적으로 다르게
#: 한 점이라 스케일업 위험이다 — 벽·임펠러 재질의 영향은 P-6(라이닝 민감도)에서
#: 따로 본다.
PILOT_WETTED_MATERIAL = "SUS316L 전 접액부 (라이닝·피복 없음)"

#: 헤드스페이스 배기 (m3/h). 수소 발생률은 원료마다 달라 아직 모르므로,
#: 배기량을 먼저 정하고 **회분 Al 의 몇 % 가 시간당 반응해도 견디는지**를
#: 역산해 판정한다. **실측 전의 설계 기준**이다 — 발생률은 P-0A(가스 포집 벤치
#: 시험)와 P-0B(PAS-1 저속 기동)에서 잰다.
PILOT_VENT_M3H = 30.0

#: 공기 중 수소 폭발하한 (체적분율).
H2_LEL_VOL = 0.04
#: 설계 상한 — 헤드스페이스 농도를 폭발하한의 25 % (1.0 vol%) 아래로 둔다.
H2_DESIGN_LEL_FRACTION = 0.25
#: 수소 검지기 경보점 — 폭발하한의 10 %.
H2_ALARM_LEL_FRACTION = 0.10
#: 실기 규모 감 잡기용 예시 — 급광 Al 의 이 비율이 반응한다고 볼 때의 발생량.
#: **설계값이 아니다.** 실제 값은 P-0 에서 잰다.
PLANT_AL_REACTION_EXAMPLE = 0.01

#: EVA 물성 — 비중(물에 뜬다), 유리전이온도(상온에서 고무 상태).
EVA_SG = 0.95
EVA_TG_C = -30.0
#: 라미네이트 EVA 의 접착·응집 강도 — MPa 급. 방식 비교에는 보수적 하한 1 MPa 를
#: 쓴다 (실제는 이보다 크다 — 비교 결론이 바뀌지 않는 쪽).
EVA_STRENGTH_MPA = 1.0
#: 로터-스테이터 간극 전단속도의 통상 상한 (1/s) — 서브mm 간극 기준.
ROTOR_STATOR_SHEAR_RATE_S = 1.0e5
#: 고체 비열 (kJ/kg·K) — Si 0.71, Al 0.90 의 질량 가중.
SOLIDS_CP_KJ_KGK = 0.73
#: 비교 기준 — 실리카사 스크러빙의 대표 입자 (µm, 비중).
REFERENCE_SAND_UM = 500.0
REFERENCE_SAND_SG = 2.65

#: 판정 기준 — 부착 EVA 는 **정한 값이 아니라 정광 품위 여유에서 나온다.** 1안 정광이
#: 보증 품위(``CONCENTRATE_GRADE_GUARANTEE``)까지 받아들일 수 있는 EVA 에서 자유 EVA
#: 한도(``EVA_FLOTATION_FEED_LIMIT``)를 뺀 나머지가 스크럽 뒤 부착 EVA 잔류 한도이고,
#: 설계 EVA 함량으로 나누면 박리 목표(제거율)가 된다 — ``plant.attrition_budget``.
#: 회분 → 연속 환산의 기준점이기도 하다. 미립 생성 기준은 AS-1 과 같은
#: ``ATTRITION_FINES_ACCEPTANCE_PP`` 를 쓴다.
ATTRITION_TARGET_OPTION = "1안"

#: 1차 적합에서 빼는 포화 제거율. 이 이상이면 잔류 부착 EVA 가 TGA 정량 하한에
#: 가까워 ``-ln(1 - X)`` 의 오차가 커진다.
PILOT_FIT_SATURATION = 0.99
...
#: 나머지는 AS-1 이 남기는 부착 EVA 몫이고, 그것이 AS-1 의 박리 목표를 정한다
#: (``ATTRITION_TARGET_OPTION``). **잠정값** — 정광 인수처 규격이 정해지면 바꾼다.
...
#: 전력·신수 절감 대안 (W3) — ES 를 이 농도에서 돌리고 부선 농도까지는 CT-1 에서
#: 공정수로 묽힌다. 슬러리 체적과 ES 앞 청수가 함께 줄지만, 같은 거품 물에 실려 가는
#: 광물이 는다. S-1 에서 두 농도의 Ag 손실을 비교한 뒤에만 택한다.
EVA_ALTERNATIVE_SOLIDS_WT = 0.20

#: 포수제 민감도 (§2.8) — ES 안에서 은이 뜨는 속도를 정상 부선(같은 약제·실기 환산)
#: 대비 이 비율로 두고 EVA 산물로 새는 Ag 를 계산한다. 회수 공정수에 남은 포수제가
#: ES 로 들어올 때의 위험을 가늠한다. 포수제 농도와의 관계는 S-1W 에서 잰다.
EVA_COLLECTOR_ACTIVITY_CASES = (0.0, 0.001, 0.003, 0.01, 0.03)
...
REAGENTS = (
    ...
    Reagent(
        ...
        solution_sg=0.81,
        addition_point="ES-1 급광 + CT-1 보충수 (트림)",
        basis="water",
        ...
        "떨어진 EVA 분리(ES-1)의 기포제를 겸한다 — ES-1 급광에 넣은 몫은 물을 따라 "
        "부선조까지 간다. CT-1 에서는 보충수(공정수) 몫만 더 넣되, 공정수에 남은 "
        "MIBC 를 빼고 거품 상태를 보며 줄인다 (트림).",
    ),
)
...
#: 여액 회수처. **정광 여액**은 공정수 탱크로 흘려보내지 않고 부선 회로의 첫 단
#: 급광으로 되돌린다 — 여포를 빠져나온 Ag 미립자가 남아 있어, 공정수로 희석해
#: 버리면 그 안의 Ag 를 그대로 잃는다. **미광 여액**은 미광 품위라 Ag 가 거의 없고
#: 유량이 커서 공정수 탱크로 보낸다. 1안 부선조는 급광 유량이 flux 로 고정이라,
#: 미광 여액까지 돌리면 최대 처리량에서 급광이 넘친다.
FILTRATE_RETURN_TO = {"mechanical": "FC-201 러퍼 급광", "rfc": "CT-1 조건조 (FC-101 급광)"}
TAILINGS_FILTRATE_TO = "공정수 탱크"

# --------------------------------------------------------------------------
# 물 계통 — 포수제 없는 물(청수)과 회수 공정수를 가른다
# --------------------------------------------------------------------------
#: 회수 공정수(농축조 월류 + 미광 여액)에는 포수제(3418A · 242)와 MIBC 가 남아 있다.
#: ES 는 체류가 길고 공기가 많아, 포수제가 조금만 들어와도 은이 EVA 산물로 뜬다
#: (§2.8 민감도). 그래서 **ES 앞에는 청수만 쓰고, 공정수는 CT-1 부터 쓴다.**
#: 청수를 ES 앞에 넣으면 그만큼이 부선 뒤 공정수로 나오고, 공정수는 ES 앞으로
#: 되돌릴 수 없으므로 케이크에 남는 몫을 뺀 전량이 블리드가 된다.
CLEAN_WATER_USERS = ("DB-1 희석수", "ES-2 세척수", "AS-1 플러싱수")
PROCESS_WATER_USERS = ("CT-1 보충수", "1안 세척수 · 2안 클리너 세척수", "약제 희석수")
#: 상류 인터페이스 — 로드밀 급수도 청수여야 한다 (공정수를 로드밀로 보내면 포수제가
#: AS-1 · ES 로 들어온다). 로드밀 배출과 함께 들어오는 물은 계통 물수지의 입력이다.
MILL_WATER = "청수 (공정수 금지)"
#: 대안 W4 — 공정수를 활성탄(GAC) 흡착탑에 통과시켜 포수제를 빼고 ES 앞에 쓴다.
#: 공탑 접촉시간(EBCT)은 용존 유기물 흡착의 통상 범위 10~20 min 의 중앙. 활성탄
#: 소모량은 공정수의 잔류 포수제 농도가 정한다 — 연쇄 시험 C-1 에서 잰다.
GAC_EBCT_MIN = 15.0

#: 용존염·미립자 축적을 막는 **최소** 블리드 — 농축조 월류수의 이 비율. ES 앞을
#: 청수로 돌리면(W1 · W3) 블리드는 이 값이 아니라 ES 로 들어간 청수가 정한다.
