...
def render(design: PlantDesign | None = None) -> str:
    ...
    add("### 현재 공장 원료 적용 경계")
    add("")
    add("문헌 원료는 패널에서 분리한 셀 분획을 습식 로드밀로 분쇄한 재료다. 현재 검토 중인 "
        "고속분쇄기-다단체-열처리 또는 attrition 블랙파우더는 입도뿐 아니라 Ag 노출도, "
        "EVA·백시트·탄화잔사, Cu·Al 함량과 펄프 레올로지가 다를 수 있다. 따라서 P80 하나만 "
        "맞는다고 문헌 성능을 이월하지 않으며, 실제 원료 characterization과 locked-cycle "
        "결과가 확인되기 전의 회수율·품위는 **참조 계산값**이다.")
    add("")
    ...
    # 2. 1안 --------------------------------------------------------------
    add("## 2. 1안 (기술 대안) — 세척수 bias 연속 부선조 1단")
    add("")
    rd = rfc.design
    add(_table(
        ["항목", "값", "비고"],
    ...
    # 3. 2안 --------------------------------------------------------------
    add("## 3. 2안 (현재 프로젝트 기본안) — 기계식 러퍼 · 스캐빈저 · 클리너 3단")
    add("")
    ...
    add("")
    add("**현재 프로젝트 권고 — 2안 기계식 3단.** 기존 공정과의 연결, 범용 장비 조달, "
        "지식재산 제약 회피와 러퍼·스캐빈저 공용화의 제작 이점을 우선한다. 1안은 계산상 "
        f"회수율이 {(rp.recovery('Ag') - mp.recovery('Ag')) * 100:.1f} %p 높고 전력은 "
        f"{(1 - rfc.installed_kw / mech.installed_kw) * 100:.0f} % 낮아 유망하지만, 실제 원료 "
        "파일럿과 실시권 확인 전에는 제작 성능보증에 사용하지 않는다.")
    add("")
    ...
    add("**pH 조정제·황화제·억제제를 쓰지 않는다.** 디티오포스핀산계 포수제가 금속 Ag 표면에 "
        ...
        "상시 pH·ORP 제어는 불필요하다. 다만 실제 블랙파우더의 열처리잔사·재순환수 영향을 "
        "확인할 수 있도록 pH 계측과 예비 약품주입 연결구는 남긴다.")
    add("")
