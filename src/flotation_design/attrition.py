"""어트리션 스크러버 (attrition scrubber) — EVA 박리 설비.

로드밀 배출 슬러리를 **고농도 그대로** 받아 입자끼리 문질러, 셀 분획 입자
표면에 붙은 **EVA(봉지재)를 떼어낸다.** 그 뒤 희석박스에서 부선 농도로 묽혀
조건조로 보낸다. 분쇄기가 아니다 — 목표는 입자를 깨는 것이 아니라 **표면에
붙은 EVA 를 떼는 것**이다.

역할의 경계
----------
이 설비가 하는 일은 **EVA 박리 하나**다. 은(Ag)을 띄우는 것은 뒤의 부선조가
한다. 그래서 이 설비의 성능은 부선 성적(회수율·품위·약제)이 아니라
**부착 EVA 제거율**과 **미립 생성량**으로 판정한다.

- 부착 EVA 제거율 — 스크럽 전후 가라앉은 분획의 EVA(TGA) 비교
- 미립 생성 — 스크럽 전후 −10 µm 질량분율 증가 (``ATTRITION_FINES_ACCEPTANCE_PP``)

무엇을 약속하지 않는가
--------------------
**이 원료(31~75 µm 미분)에서 EVA 가 기계적으로 떨어진다는 시험 근거는 아직
없다.** 박리 성능은 시험 셀 PAS-1(``attrition_pilot``)에서 확정한다. 부선
계산은 어트리션과 무관하게 문헌 원료 기준으로 하고, EVA 를 뗀 효과를 부선
성적에 반영하지 않는다 (``ATTRITION_PERFORMANCE_CREDIT = 1.0``). 대신
(a) 전량 바이패스 배관을 두어 없는 것처럼 운전할 수 있게 하고, (b) 박리를
정량화할 시험 절차와 합격 기준을 설계에 포함한다. 시험에서 박리가 확인되지
않으면 바이패스로 두거나 열분해로 간다 — 이 설비는 공짜가 아니다.
...
class DilutionBox:
    ...
    어트리션 배출은 70 wt% 짜리 반죽이라 그대로는 풀어지지 않는다. 희석박스는
    (a) 반죽을 풀고, (b) 어트리션 바이패스 배관의 합류점이 되며, (c) 밀도계로
    희석수를 조절해 ES 급광 농도를 **7 wt% 로 고정**한다. 부선 급광은 CT-1
    보충수(공정수)가 맞춘다.

    Attributes:
        dilution_water_m3h: 추가로 넣는 물 — 설계 유량 (출구 농도 기준, 순환류
            공제 없음). **청수(포수제 없는 물)** 로 넣는다. 회수 공정수에는 포수제가
            남아 있어 ES 에서 은이 EVA 산물로 뜬다.
    """
