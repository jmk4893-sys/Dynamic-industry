...
python -m unittest discover -s tests -t .                               # 테스트 (483건)
```
...
- 전기 인입 **기존 부지 1,200 kW 인입에서 분기** — 부지에 이미 인입이 있으므로 자체 수전설비를 세우지 않는다. 이 플랜트는 그 계통의 부하 하나(계약 기준 **22.4 %** · 여유 931.8 kW)이며, 수용률이 전부 1.0 이 되는 최악(266.0 kW)에도 22.2 % 다. 세울 큐비클도 전기실도 없다(0 × 0)
- 분기점은 **부지 저압 배전반 380 V** — 저압 분기는 허용전류가 아니라 **전압강하**가 거리를 묶는다. 4C×240 mm² 기준 한계 **162 m**(강하 3 %), 300 mm² 면 188.8 m. 넘으면 22.9 kV 로 따고 플랜트 옆에 국소 변압기 300 kVA(2,600 × 3,600 mm)를 세운다 — 그 전환은 `TAP_AT_LOW_VOLTAGE` 한 줄이다
- **부지 배전반까지의 실거리는 발주처 실측 사항이라 지어내지 않는다** — `wiring.SITE_BOARD_TO_MDB_MM = None` 이면 도면도 길이 대신 "미확정 (한계 162 m)" 를 적는다
- 부하 집계 설치 **266.0 kW** · 수요 **198.7 kW** · 계약 **268.2 kW / 298.1 kVA** · 주회로 335.4 A · 4C×**240 mm²** · ACB 400 AF·400 AT · 역률개선 35 kVar — 유리제거셀 IR 뱅크 175 kW 가 들어오며 전처리만일 때(68.0/50.2/100 AT/75.4 kVA)의 4 배가 됐다. 계약전력이 저압 공급 상한 100 kW 를 넘는다는 REV.23 판정은 그대로 유효하며, 그 수전설비가 부지 쪽에 이미 있을 뿐이다
- 배선 MDB-101 벽부 x=42,500(수요 가중 부하중심) · 전력 케이블 298.4 m · EtherCAT 체인/FSoE 링 (EL-1005~1008) — 전기 시트는 행이 늘면 실측해서 프레임과 viewBox 를 같이 키운다(`fitElectricalSheet`)
- 전동기 서보 **36축** 27.9 kW + 인버터·기어드 — 축 일람·라이브 동작 확인 (설계·PLC·검증 탭)
- 소음·진동 근접 88 → 70 dBA·통로 최악 59.9 dBA — 저감 장치 4종 반영 (NV-1009)
- 열수지 실내 **54.6 kW**·환기 **33,000 m³/h** — HPU 오일쿨러 2기·반 열교환기 3면, IR 배기(45.9 kW)와 냉각 후드(85.3 kW)가 전제다. 후드가 현열을 못 잡으면 환기가 83,500 m³/h 로 뛴다 (TH-1010)
