...
- **들어 올린 채로 설치된 설비 위를 넘지 않는다** — 넘으려면 최고 고정점 5,150 + 통과 300 + 물건 + 슬링 + 후크블록 = **12,970** 이라 12 m 천장에 안 들어간다. 그래서 **하류(GRM)부터 세워 상류(AFU)로 온다**(발주처 확인) — 공정 흐름의 반대다. **출입구는 투입방향(상류 AFU)** 이고 건축이 이미 그렇게 설계돼 있다(발주처 확인). 반입 동선이 곧 **아직 안 세운 장비 밴드**(폭 7,100)라서, 세우는 순간 그만큼 길이 사라지기 때문이다. 통로(유효 900)는 사람이 다니는 길이지 반입 동선이 아니다 — 최대 모듈인 반전 카세트가 폭 **2,900** 이다. 크레인 사양이 아니라 시공 순서로 푸는 문제다
