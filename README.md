...
python -m unittest discover -s tests -t .                               # 테스트 (459건)
```
...
src/pv_preprocess/
  ...
  handoff.py        후단 박리 라인 인계 — 자세·치수·처리율 경계 조건 대조
...
- 후단 인계 — 유리 버퍼가 [DG-HK 2400 박리 라인](docs/drawings/pv-delam-tandem.html)으로 이어진다. 자세는 맞고(유리면 ↓), 처리율은 **B안 채택으로 해소**(66.0 유입 vs 70.6 처리, 여유 +4.6 장/h), 치수만 아직 안 맞는다(상한 2,500×1,400 vs 2,400×1,200) — `handoff.py`
- 격차 흡수 두 안 — **B안** 후단 개선(칼날 40→60 mm/s·인계 10→6 s → 70.6 장/h, **버퍼에 연결된 구성**) / **C안** 전처리 감속(방출 보류 +21.3 s → 택트 69.4 s, 미채택). `PYTHONPATH=src python tools/build_handoff_variants.py out/` 로 각 안이 반영된 미니앱 두 벌을 찍는다
- 채택 근거 — 인계 10 s 를 그대로 두면 균형에 필요한 칼날이 60.6 mm/s 로 상한 60 을 넘는다. 인계를 6 s 로 줄여야 필요치가 55.6 으로 내려와 성립한다. 버퍼는 이제 밀린 것을 쌓는 곳이 아니라 후단 정지를 0.76 h 버티는 완충이다
