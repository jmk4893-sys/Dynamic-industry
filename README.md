...
- 영구설비 **58,800 × 8,300 × 5,050 mm** — 셀별 GA 외형에서 파생 (전처리 44,750 + 유리제거셀 GRM-401 14,050)
...
- **지지·장착** — 3D 메시 1,921개를 인접 30 mm 로 묶어 바닥까지 하중 경로를 따라가면 접지 성분 1개, 예외 2덩어리(로봇이 든 패널·레이저 투영선)뿐이다. REV.25 까지는 메시 1,765개 중 덩어리 **77개(144메시)** 가 떠 있었다 — 5단 랙 24메시가 바닥에서 600 mm 뜬 채였다
...
- 전기 인입 **기존 부지 1,200 kW 인입에서 분기** — 부지에 이미 인입이 있으므로 자체 수전설비를 세우지 않는다. 이 플랜트는 그 계통의 부하 하나(계약 기준 **23.8 %** · 여유 914.5 kW)이며, 수용률이 전부 1.0 이 되는 최악(280.8 kW)에도 23.4 % 다. 세울 큐비클도 전기실도 없다(0 × 0)
- 분기점은 **부지 저압 배전반 380 V** — 저압 분기는 허용전류가 아니라 **전압강하**가 거리를 묶는다. 4C×240 mm² 기준 한계 **152 m**(강하 3 %), 300 mm² 면 177.4 m. 넘으면 22.9 kV 로 따고 플랜트 옆에 국소 변압기 500 kVA 를 세운다 — 그 전환은 `TAP_AT_LOW_VOLTAGE` 한 줄이다
- **부지 배전반까지의 실거리는 발주처 실측 사항이라 지어내지 않는다** — `wiring.SITE_BOARD_TO_MDB_MM = None` 이면 도면도 길이 대신 "미확정 (한계 152 m)" 를 적는다
- 부하 집계 설치 **280.8 kW** · 수요 **211.5 kW** · 계약 **285.5 kW / 317.3 kVA** · 주회로 357.1 A · 4C×**240 mm²** · ACB 400 AF·400 AT · 역률개선 35 kVar. 스마트 팩토리 14.8 kW 가 붙으며 **400 AT 여유가 3.9 kW 만 남았다** — 다음에 이만한 부하를 하나 더 붙이면 차단기·주회로가 한 단 올라간다
- 배선 MDB-101 벽부 x=42,500(수요 가중 부하중심) · 전력 케이블 331.1 m · EtherCAT 체인/FSoE 링 (EL-1005~1008) — 시트는 행이 늘면 실측해서 프레임과 viewBox 를 같이 키운다(`fitSheet`)
