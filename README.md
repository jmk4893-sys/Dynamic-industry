...
- **CRN-901 5 t 천장크레인** (천장고 12,000 확정) — 후크가 올라가는 높이는 천장고가 아니다. 천장 12,000 − 보 하면 여유 300 − 거더·트롤리 950 = 주행레일 상면 **10,750**, 여기서 C 치수 1,050 을 빼야 후크 최고 **9,700** 이 나온다. 인양물 위로 슬링 2,000 + 후크 블록 500 이 더 붙으므로 4,500 짜리 BFC 반전 카세트를 들려면 후크가 7,200 까지 올라가야 한다
- 용량은 최중량 단품이 정한다 — BFC 반전 카세트 **1,980 kg** 에 5 t 이면 여유 **2.53배**(중량은 단면·재질에서 낸 계획값이라 벤더 GA 를 받을 여유가 필요하다). 후크 높이는 VG-101 비전보(5,150)가 정하고 여유 **1,850 mm** 다
...
- **들어 올린 채로 설치된 설비 위를 넘지 않는다** — 넘으려면 최고 고정점 5,150 + 통과 300 + 물건 + 슬링 + 후크블록 = **12,450** 이라 12 m 천장에 안 들어간다. 반입은 통로(Y 7,100–8,300)로 제자리 옆까지 가서 내리고 **하류(GRM)부터 세워 상류로 온다** — 크레인 사양이 아니라 시공 순서로 푸는 문제다
...
- **AI 적용 검토 12건** — A 3건(지금 데이터로 착수) · B 5건(신규 계측기 필요) · C 2건(되지만 이득이 작아 권고하지 않음) · D 2건(지금은 불가). 라벨은 연 149,400장이 공정에서 나오고, 가장 희소한 전손(4,980장/년)이 착수 시점 **2.4개월**을 지배한다
