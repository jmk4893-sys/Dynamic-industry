...
TAKT = 53.6                       # s 라인 사이클 — 피치가 이보다 짧아야 한다
DWELL_MAX = TAKT * IR.DECKS       # 268 s 체류 상한
T_TARGET = c("T_TARGET")          # 140 ℃ 냉점 목표
T_BACK_MAX = 165.0                # PVDF 융점
DT_GLASS = 21.0                   # K 유리 허용 면내 편차 (CAL-001 R1)
Q_PANEL = 65_000 / IR.DECKS       # W/장 — 1D 열수지가 정한 값

# ── 현행과 개선 ──────────────────────────────────────────────────────
NOW_N, NOW_SPAN, NOW_LEN = 7, 2.48, 1.300        # 콘솔 도면 · 카탈로그 P-002-18
NEW_LEN = 2.200                                   # 발열장 — 단자는 벽 밖
NEW_X = (-1.340, -0.948, -0.475, 0.0, 0.475, 0.948, 1.340)
...
def run():
    ...
    return [
        Result("IR1", "현행 배치의 면내 온도편차", a["spread"], "K", DT_GLASS,
               ...
               f"끝단 여유가 50 mm 뿐이라 폭 가장자리가 중앙의 69 % 다. "
               f"**유리가 깨진다**"),
        ...
        Result("IR3", "현행 배치의 소요 체류시간", a["dwell"], "s", DWELL_MAX,
               ...
               f"평균이 아니라 **냉점**이 140 에 닿아야 하므로 설계 222.6 s "
        ...
        Result("IR4", "개선 배치의 면내 온도편차", b["spread"], "K", DT_GLASS,
               ...
               f"발열장 {NEW_LEN*1000:.0f} · 위치 ±{{0·475·948·1340}} · "
               f"내피 ρ {RHO_POLISHED:.1f}. 길이방향 {dx:.1%} · 폭방향 {dy:.1%} 로 "
               f"두 방향이 균형을 이룬다 (현행은 18 % : 35 %)"),
        ...
        Result("IR6", "개선 배치의 소요 체류시간", b["dwell"], "s", DWELL_MAX,
               ...
               f"설계 222.6 s 보다 {b['dwell']-222.6:.0f} s 길어진 것은 "
...
def requirements() -> list[Req]:
    ...
    _, ex = run()
    return [
        Req("RIR1", "IR 램프 발열장", f"{NEW_LEN*1000:.0f} mm (현행 1,300)",
            "부품 카탈로그 P-002-18 · 구매 사양",
            f"면내 결손의 정체가 램프 축 방향이라 **길이가 유일하게 크게 듣는 "
            f"기하 변수**다. 1,300 은 패널 폭 1,200 에 끝단 여유 50 mm 뿐이고, "
            f"그 상태로는 어떤 배치·어떤 출력제어로도 창이 닫히지 않는다"),
        Req("RIR2", "뱅크 램프 x 위치",
            "±" + "·".join(f"{x*1000:.0f}" for x in NEW_X if x >= 0) + " mm",
            "콘솔 도면 · F-002 제작도",
            "균등 배치가 아니다. 끝을 바깥으로 밀어 패널 끝단(±1,200)을 "
            "넘긴다 — 끝 램프가 패널 안쪽에 있으면 그 바깥은 아무도 데우지 "
            "않는다. 인접 뱅크는 **같은 위치**를 쓴다 (엇갈리면 나빠진다)"),
        Req("RIR3", "내피 반사율", f"ρ ≥ {RHO_SPEC:.1f} · 검사·정비 항목",
            "사양서 · ITP · 정비 지침",
            f"연마 STS304 #400 이 {RHO_POLISHED:.1f} 를 주므로 요구는 그 절반이다. "
            f"그러나 **오손되면 유리가 깨진다** — 지금까지 내피는 내식·복사면으로만 "
            f"적혀 있었고 반사율은 아무도 요구하지 않았다. 정기 청소와 "
            f"광택 검사가 성능 항목이 된다"),
        Req("RIR4", "2,200 mm 램프의 지지·관통 상세",
            "처짐 · 실링 · 그림자를 함께 푸는 상세",
            "상세설계 (이 검토가 확정하지 않는다)",
            f"단자를 측벽 밖으로 빼는 것은 길이를 벌기 위해서만이 아니라 "
            f"석영 단자가 고온부 밖에 있어야 수명이 서기 때문이다. 대신 "
            f"뱅크당 14 · 총 84 개소의 관통을 밀폐해야 하고, 2.2 m 수평 "
            f"석영관은 중간 지지가 필요할 수 있다. 지지대는 **위에서** 잡아 "
            f"그림자가 패널이 아니라 반사판으로 가게 한다 — 아래에서 잡으면 "
            f"이 검토가 애써 만든 균일도를 지지대가 도로 깬다"),
...
def report() -> str:
    ...
    add(f"   램프        발열장 {NEW_LEN*1000:.0f} mm · 2.5 kW · 뱅크당 "
        f"{len(NEW_X)} 등 (총 {IR.LAMPS} 등 유지)")
    add(f"   위치        x = " + " · ".join(f"{x*1000:+.0f}" for x in NEW_X) + " mm")
    add(f"   단자        측벽 관통 · 챔버 밖 소켓 (뱅크당 14 · 총 84 개소)")
    ...
    add("")
    add("=" * 78)
    add("전 항목 만족" if not bad else f"★ {bad} 항목 초과 (현행 배치 IR1~IR3)")
    add("=" * 78)
    return "\n".join(L)
