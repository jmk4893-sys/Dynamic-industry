"""열수지를 닫는다 — 손실 예산의 나머지는 어디로 갔는가 (열해석 R5).

열해석이 요구 R5 를 남겼다: **벽 손실은 5.6 kW 로 손실 예산 42 kW 의
13 % 뿐이고, 나머지 36 kW 의 행방을 아무도 세지 않았다.**

세어 보니 **질문 자체가 틀려 있었다.** 두 군데가 어긋난다.

(여기 적은 숫자는 REV.21C — 포락선 2,500 × 1,400 · 48 등 · 120 kW — 의
값이다. 보고서에 나가는 값은 전부 run() 이 콘솔 상수에서 다시 낸다.)

── ① 78 kW 는 계약 처리량의 값이 아니다 ────────────────────────────
콘솔은 `유효 = 정격 120 kW × η 0.65 = 78 kW` 를 쓰고, 그 78 kW 를
"패널이 받는 열" 로 놓는다. 그런데 78 kW 는 **열공정 한계 79.9 장/h**
에서 패널이 받는 값이다. 라인은 계약 **58 장/h** 로 돈다 — 그때 패널이
가져가는 것은 장당 3.516 MJ × 58 = **56.6 kW** 다.

    120 kW − 78 kW = 42 kW      ← 정격과 열공정 한계를 뺀 값
    87.2 kW − 56.6 kW = 30.5 kW ← 계약 처리량에서 η 0.65 가 뜻하는 손실

"42 kW" 는 서로 다른 두 운전점에서 하나씩 가져온 숫자였다.
...
세면 아래 표가 되고, 남는 항은 없다. 36 kW 는 새는 것이 아니라 **돌고
있었다.**
...
RATED_KW = TH.RATED_KW               # 120 kW 설치정격 (48 등 × 2.5 kW)
ETA_ASSUMED = TH.ETA                 # 0.65 콘솔 MODEL 기본값
USEFUL_KW = RATED_KW * ETA_ASSUMED   # 78 kW 콘솔의 '유효'
LOSS_BUDGET = TH.LOSS_BUDGET         # 42 kW = 정격 − 유효 — R5 가 들고 온 예산 (T12)
...
def fork(t_rest: float = T_AMB, takt: float = TAKT) -> float:
    ...
    **처음에 200 배 틀렸다.** 포크 42 kg 이 매 사이클 (140−40)/3 만큼
    ...
    필요하다. IR 설치정격(120 kW)보다 크다. 챔버가 줄 수 없는 열이었다.
...
def run():
    ...
    return [
        Result("HB1", "계약 처리량의 패널 엔탈피", b["panel"], "kW",
               ...
               f"라인은 계단 칼날이 정하는 {RATE_CONTRACT:.0f} 장/h 로 도는데 "
               f"'{RATED_KW:.0f} − {USEFUL_KW:.0f} = {LOSS_BUDGET:.0f} kW' 는 "
               f"**두 운전점에서 하나씩 가져온 뺄셈**이었다"),
        Result("HB2", "세어 낸 손실 합계", b["loss"], "kW", b["assumed_loss"],
               ...
               f"{b['fork']:.2f}. **남는 항이 없다** — "
               f"{LOSS_BUDGET-b['wall']:.0f} kW 는 새는 것이 "
        ...
        Result("HB8", "포크가 퍼올려 나가는 열", b["fork"], "kW", 2.0,
               ...
               f"그 온도변화는 5 초에 140 kW 를 요구하는데 정격이 "
               f"{RATED_KW:.0f} kW 다. "
...
def requirements() -> list[Req]:
    ...
    return [
        Req("RHB1", "손실 예산의 정의를 바꾼다",
            ...
            f"'정격 {RATED_KW:.0f} − 유효 {USEFUL_KW:.0f} = 손실 "
            f"{LOSS_BUDGET:.0f} kW' 는 두 운전점을 뺀 값이라 "
            f"물리적 의미가 없다. 정상상태에서 계를 떠나는 것은 "
            f"{b['loss']:.1f} kW 이고, 평균 IR 소요는 {b['p_ir']:.1f} kW 다. "
            f"**설치정격 {RATED_KW:.0f} kW 는 승온 속도가 정하지 정상 소비가 정하지 "
...
def report() -> str:
    ...
    b = ex["b"]
    add("=" * 78)
    add(f"DG-HK60C 열수지 — 손실 예산 {LOSS_BUDGET:.0f} kW 의 행방 (R5)")
    add("=" * 78)
    add("")
