...
2,500 mm 가 통째로 서야** 두 문을 다 닫을 수 있다. 0.30 m 짜리 격리실은
기하학적으로 있을 수 없다.
...
# ── 격리실 ───────────────────────────────────────────────────────────
VEST_D = c("PANEL_L") + 0.30                 # 2.80 m 두 문 사이 — 패널이 통째로 선다
VEST_ADD = c("PANEL_L")                      # 2.50 m 출력측에 새로 드는 길이
...
def run(other_loss_kw: float = None, panel_kw: float = None):
    ...
    return [
        ...
        Result("AL4", "격리실을 더했을 때의 이득 (안 D)", C["kw"] - D["kw"], "kW", 2.0,
               f"라인을 {VEST_ADD*1e3:,.0f} mm 늘릴 값 — 2 kW 를 넘어야 검토할 가치가 있다",
