/* 후단 도면집이 **정말로 한 벌씩만 보이는지** 브라우저로 확인한다.
 *
 * 네 문서를 한 장에 담는 화면이라 파이썬 시험이 못 보는 실패 방식이 있다.
 ...
 *        (인자를 안 주면 docs/drawings/pv-afr-hub.html 을 본다)
 */
...
const file = process.argv[2] || 'docs/drawings/pv-afr-hub.html';
if (!existsSync(file)) {
  console.error(`✗ 문서가 없다: ${file}\n  PYTHONPATH=src python tools/build_afr_hub.py 를 먼저 돌릴 것`);
  process.exit(2);
}

//: 탭 키 → [기대하는 문서 제목의 일부, 캔버스가 있어야 하는가]
//  넷 다 3D 도면이라 캔버스가 있어야 한다 — 없으면 액자가 열리다 만 것이다.
const WANT = {
  scene: ['AFR-101 알루미늄 프레임 제거장치', true],
  closeup: ['AFR-101 부품 확대도', true],
  grind: ['SG-301 연마 작동 확대도', true],
  inspect: ['GI-302 · GI-303 검사 작동 확대도', true],
};
...
for (const key of Object.keys(WANT)) {
  ...
  await page.waitForTimeout(5000);   // 넷 다 3D 라 넉넉히 준다
  ...
  // 띠는 구간이 아니라 **점유**다 — 「… s / AFR … s」 꼴이어야 한다.
  if (!/[\d.]+ s \/ AFR [\d.]+ s$/.test(got.span)) why.push(`점유 표시가 «${got.span}»`);
...
console.log(bad ? `✗ ${bad} 건` : `✓ 후단 ${Object.keys(WANT).length} 도면이 한 벌씩만 열린다`);
