...
def requirements() -> list[Req]:
    ...
    return [
        Req("R1", "패널 면내 온도편차",
            f"≤ {DT_INPLANE:.0f} K (설계목표 ≤ {DT_INPLANE*0.7:.0f} K)",
            "IR 뱅크 배치 · 반사판 · 파일럿 PT-04",
            f"두께 방향은 여유가 {SIG_GL/_sigma(soak(FLUX)['dt_glass']):.0f} 배지만 "
            f"**면내 편차는 아무도 보지 않았다.** 중앙이 뜨겁고 가장자리가 찬 "
            f"분포에서 가장자리 인장은 대략 ½·Eα·ΔT 이므로 허용 {SIG_GL:.0f} MPa "
            f"에서 {DT_INPLANE:.0f} K 다 (판유리 실무의 어닐링 40 K 규칙과 같은 "
            f"모델). 클램프로 잡으면 완전구속이 되어 {DT_INPLANE_FIX:.0f} K 로 "
            f"내려가니 **소킹 중에는 패널을 구속하지 않는다**. 1차원 해석은 이 "
            f"편차를 못 낸다 — 램프 배치와 반사판이 정한다. 현재 계측은 단당 "
            f"K 열전대 3점 + 표면 IR 1점이라 편차를 보기엔 성기다"),
