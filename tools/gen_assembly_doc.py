...
ERECTION = [
    ...
    ("4", "후단 설비", ["M-006", "M-008", "M-009"],
     "공정 상류가 확정된 뒤에 하류를 맞춘다. 셀모듈 경로와 유리 경로가 상류 위치에서 나온다.",
     "카세트 반출 · 횡인출 · 검사"),
    ("5", "유틸리티", ["M-011", "M-012"],
...
TOOLS = [
    ...
    ("다이얼게이지·마그네틱베이스", "0.01 mm", "캐리어 빔 취부면 · 레일 직진도 · 상판 평면도"),
    ...
    ("체인블록·슬링", "2 t 이상", "가열실 기둥 222 kg · 크로스빔 · 칼날 카세트"),
...
MISTAKES = [
    ...
    ("마찰접합면에 도장을 한다", "J3(갠트리 크로스빔)은 <b>마찰</b>로 힘을 받는다. 도장하면 μ 가 떨어져 미끄러지고, "
     "미끄러지면 칼날이 돌아 좌·우 계단의 선후가 어긋난다 (계단 80 ± 0.5). 접합면은 Sa 2½ 로 두고 도장하지 않는다."),
...
<section id="a2"><h2><span class="sn">2</span>안전 — 조립 중에 다치는 자리</h2>
  <div class="clause"><div class="n">2.1</div><div class="c">
    ...
    <div class="warn"><strong>중량물이 첫째다.</strong> 가열실 기둥 한 본이
      ...
      칼날 카세트가 <strong class="m">{F.c('CASS_MASS'):,.0f} kg</strong> 이다. <strong>25 kg 이 넘는 것은 사람 손으로 들지 않는다</strong> —
