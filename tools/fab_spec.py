...
# 자중 (설계 가정 — 제작사 중량표로 확인한다).
# 가열실은 벽체 4겹과 골조에서 낸 값이고, 나머지는 부재표에서 역산한 개산이다.
CHAMBER_AREA = 2 * (3.78 * 4.725) + 2 * (2.56 * 4.725) + 2 * (3.78 * 2.56)
M_CHAMBER = (
    CHAMBER_AREA * (3.2e-3 * 7850 + 2.0e-3 * 7930)      # 외피 SS400 + 내피 STS304
    + CHAMBER_AREA * 0.100 * 100                        # 미네랄울 100 t · 100 kg/m³
    + 1500                                              # 골조 (기둥 4 · 거싯 · 크로스빔)
    + c("DECKS") * 120                                  # 데크
    + (c("DECKS") + 1) * 80                             # IR 뱅크
    + c("LAMPS") * 2                                    # 램프
)
M_GANTRY = 1450     # KG-101 크로스빔 + 대차 2 + Z슬라이드 2 + 칼날 2 + 카세트 2
M_TABLE = 900       # VT-101 상판 + 매니폴드 + 기둥 4
M_WINDER = 620      # WR-101 문형 + 드럼 + 서보
...
MEMBERS = [
    ...
    ("M-004", "VT-101 상판", "PL 20 (리브 PL 10 @400)", "SS400", "진공 −65 kPa 면외 압력"),
    ("M-004", "테이블 기둥", "□-150×150×6", "SM490A", "박리 추력 전도"),
